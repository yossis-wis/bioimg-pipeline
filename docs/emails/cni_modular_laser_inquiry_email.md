...
> **Editing note:** this email can be shortened by deleting **“Background / funding constraint”** and **“Rationale / context”**, while keeping everything from **“Request (action needed)”** onward.
...
**Subject (suggested):** Quote request: 640 + 488 nm modules (SM-fiber vs wide-linewidth+MMF) — model, price, delivery to Israel
...
At this stage we do **not** need AOMs or ultra-fast gating; **CW output is fine**.
...
- **Approach B (wide-linewidth + MMF concept):** use **wide-linewidth lasers** intended to be combined into a **single large-core multimode fiber (MMF)** to help homogenize illumination.
...
**Option B: wide-linewidth outputs (for MMF illumination concept)**
- **640 nm:** ~1–2 W, **wide linewidth** (roughly Δλ ~2–20 nm), ideally **multi-longitudinal-mode** (many spikes simultaneously present)
- **488 nm:** ~100 mW, **wide linewidth** (≥ ~2 nm)
...
![](../figures/approach_a_single_mode_fiber.svg)

```text
[640 nm SM fiber-coupled] --FC/APC--> [fiber collimator] --\
                                                         >-- [dichroic combiner] --> [to microscope / objective]
[488 nm SM fiber-coupled] --FC/APC--> [fiber collimator] --/
```

### Approach B diagram (wide linewidth + MMF delivery)

![](../figures/wide_linewidth_mmf_widefield_system.svg)

```text
[640 nm wide-linewidth] --\
                           >-- [combine] --> [MMF coupler] --> [square step-index MMF (200–400 µm; ~3 m)] --> [to microscope / objective]
[488 nm wide-linewidth] --/

(optional) add a ~10 kHz fiber agitator / scrambler for longer exposures (≥5 ms)
```
