...
def compute_phasors(cfg: InstantaneousPhasorSumConfig) -> InstantaneousPhasorSumResult:
    ...
    phi0 = np.zeros((int(cfg.n_wavelengths), int(n_paths)), dtype=np.float64)
    if cfg.add_random_initial_phase:
        phi0 = rng.uniform(0.0, 2.0 * np.pi, size=phi0.shape).astype(np.float64)
